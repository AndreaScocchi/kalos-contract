...
- SELECT: 
  - Own bookings: `user_id = auth.uid()` OR
  - Linked client bookings: `client_id` references `clients.id` where `clients.profile_id = auth.uid()` OR
  - Staff: `is_staff()`
- **NO direct writes allowed**: Grants revoked for INSERT/UPDATE/DELETE on `bookings` table.
- All writes must use RPC:
  - User App: `book_lesson()`, `cancel_booking()`
  - Gestionale for CRM clients: `staff_book_lesson()`, `staff_cancel_booking()`
...
- Ownership:
  ...
  - XOR constraint enforced in DB.
- SELECT:
  - Own subscriptions: `user_id = auth.uid()` OR
  - Linked client subscriptions: `client_id` references `clients.id` where `clients.profile_id = auth.uid()` OR
  - Staff: `is_staff()`
- Writes: staff-only (INSERT/UPDATE/DELETE via `is_staff()` policy).
- User App reads only account-owned subscriptions.

### subscription_usages
- SELECT:
  - Staff: `is_staff()` OR
  - Owner via subscription: subscription exists where `subscription.user_id = auth.uid()` OR
  - Linked client owner: subscription exists where `subscription.client_id` references `clients.id` and `clients.profile_id = auth.uid()`
- **NO direct writes allowed**: Grants revoked for INSERT/UPDATE/DELETE on `subscription_usages` table.
- Writes only via:
  - RPC side effects (`book_lesson`, `cancel_booking`, `staff_book_lesson`, `staff_cancel_booking`)
  - Staff-only policy (for exceptional manual adjustments)
...
### staff_book_lesson(p_lesson_id uuid, p_client_id uuid, p_subscription_id uuid DEFAULT NULL)
- SECURITY DEFINER
...
- Reactivation handling:
  - If reactivating a canceled booking, removes any existing CANCEL_RESTORE usage entry (delta=+1)
  - Ensures proper accounting by checking for existing BOOK usage entry (delta=-1)
  - Prevents double accounting of entries
- Returns jsonb: `{ ok: boolean, reason?: string, booking_id?: uuid }`
...
- **All booking/subscription_usages writes MUST go through RPC** - direct INSERT/UPDATE/DELETE is blocked by grants.
...
- **No duplicate RLS policies**: Each table-operation combination should have exactly one policy.