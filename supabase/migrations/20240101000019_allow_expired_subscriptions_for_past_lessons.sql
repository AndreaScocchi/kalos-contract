-- Migration 0019: Allow expired subscriptions for past lessons in staff_book_lesson
--
-- Obiettivo: Permettere allo staff di usare abbonamenti scaduti per prenotare lezioni passate.
-- Questo consente di registrare prenotazioni retroattive anche quando l'abbonamento è scaduto,
-- purché lo status sia ancora 'active'.
--
-- Modifiche:
-- - La funzione staff_book_lesson ora permette abbonamenti scaduti per lezioni passate
-- - Per lezioni future, mantiene il controllo originale (abbonamento attivo e non scaduto)
-- - Per lezioni passate, richiede solo che l'abbonamento abbia status = 'active'
...
begin
  ...
  if p_subscription_id is not null then
    ...
    -- For past lessons, allow expired subscriptions (status must still be 'active')
    -- For future lessons, require subscription to be active and not expired
    if v_client_profile_id is not null then
      -- Client has account: check both client_id and user_id (profile_id)
      if v_starts_at < v_now then
        -- Past lesson: allow expired subscriptions (only check status = 'active')
        select *
        into v_sub
        from subscriptions
        ...
        -- Future lesson: require active and not expired subscription
        select *
        into v_sub
        from subscriptions
        where id = p_subscription_id
