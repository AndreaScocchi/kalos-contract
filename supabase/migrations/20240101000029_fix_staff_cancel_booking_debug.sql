-- Migration 0029: Fix and debug staff_cancel_booking subscription lookup
-- 
-- Obiettivo: Correggere la logica di ricerca della subscription in staff_cancel_booking
-- e assicurarsi che funzioni correttamente anche quando subscription_id è NULL sulla booking
--
-- Problema: La funzione potrebbe non trovare la subscription se subscription_id è NULL
-- e non c'è un record subscription_usages con delta = -1

-- ============================================================================
-- MIGLIORARE staff_cancel_booking con logica più robusta
-- ============================================================================
...
declare
  ...
  v_client_id uuid;
  v_subscription_usage_found boolean := false;
begin
  ...
  -- Update booking status FIRST (this will trigger the AFTER UPDATE trigger)
  -- But we also need to handle it here to ensure it works
  update bookings
  set status = 'canceled'
  where id = p_booking_id;
  ...
  if found then
    v_subscription_usage_found := true;
  end if;
  ...
  return jsonb_build_object(
    ...
    'usage_found', v_subscription_usage_found
  );
end;
$$;
